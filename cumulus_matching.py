...
def cumulus_run(file_1, file_2, path):
    ...
    if xls in file_1:
        cumulus = pd.read_excel(file_1)
    elif csv in file_1:
        cumulus = pd.read_csv(file_1)
        
    if xls in file_2:
        backfill = pd.read_excel(file_2)
    elif csv in file_2:
        backfill = pd.read_csv(file_2)
    ...
    principals.to_csv(path)


''' Turn script to function: list out what the variables have to be:
    - path as a string - line 194 - (PATH NAME)
    - cumulus file name - line 18 - (FILE_1)
    - backfill file name - line 20 - (FILE_2)    
    '''
