...
def run_knack():
    ...
    knack_extract = knack_extract[pd.notnull(knack_extract['Entity name'])]
    # Import file
    ...
    # Create dummy ID on import files
    ...
    knack_matches = knack_matches.reset_index(drop=True)
    ...
    kam = knack_matches.merge(frame, on = 'dummy_id', how = 'left')

    
    kam_m = kam[pd.notnull(kam['Compass Listing ID'])]
    
    kam_mm = kam[kam['Compass Listing ID'].isnull()] 
    ...
    research_k.to_csv('/Users/pierre.casco/backfill/Files/knack_research.csv')


'''
    change date field - month year
